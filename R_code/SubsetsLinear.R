...
regfit.full=regsubsets(fit.linear,data)
...
## Forward
regfit.fwd=regsubsets(fit.linear,data=data, nvmax=8, method ="forward")
summary(regfit.fwd)
# We see that using forward stepwise selection, the best one-variable is approved.

## Backward
regfit.bwd=regsubsets(fit.linear,data=data,nvmax=8, method ="backward")
summary(regfit.bwd)
...
regfit.best=regsubsets(fit.linear,data=data[train,], nvmax =8)
...
test.mat=model.matrix(fit.linear,data=data[test,])
