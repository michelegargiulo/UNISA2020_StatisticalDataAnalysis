...
regfit.full=regsubsets(fit.poly3,data, nvmax = 22)
...
## Forward
regfit.fwd=regsubsets(fit.poly3,data=data, nvmax=22, method ="forward")
summary(regfit.fwd)
# We see that using forward stepwise selection, the best one-variable is approved.

## Backward
regfit.bwd=regsubsets(fit.poly3,data=data,nvmax=22, method ="backward")
summary(regfit.bwd)
...
regfit.best=regsubsets(fit.poly3,data=data[train,], nvmax = 22)
...
test.mat=model.matrix(fit.poly3,data=data[test,])
