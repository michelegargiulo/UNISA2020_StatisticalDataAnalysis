...
library(glmnet)
attach(data)
x = model.matrix(fit.linear, data)[,-1] #[-1] means no intercept
...
# variables into dummy variables (approved).
...
# fit.lm=lm(final_vote~approved+comfort_score+service_score+food_score+enjoyment_score+station_score
#                         +expendiency_score, data=data) # 1 coeff = 0 (wifi_connectivity)
fit.lm=lm(final_vote~approved+comfort_score+service_score+food_score+enjoyment_score+station_score
          +expendiency_score, data=data)
coef(fit.lm)