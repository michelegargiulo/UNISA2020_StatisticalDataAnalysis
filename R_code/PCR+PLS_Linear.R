...
pcr.fit=pcr(fit.linear,data = data ,scale=TRUE, validation ="CV")
...
set.seed (1)
x = model.matrix(fit.linear,data = data)[,-1]
...
pcr.fit=pcr(fit.linear,data = data ,subset=train,scale=TRUE,
            validation ="CV")
dev.new()
# Plot MSE and RMSE 
validationplot(pcr.fit,val.type="MSEP",legendpos = "topright") 
...
# Setting scale=TRUE has the effect of standardizing each predictor (scale projection).
# Setting validation = 'CV' has the effect to use cross validation to rate M parameter
pls.fit=plsr(fit.linear,data = data, scale=TRUE, 
             validation ="CV")
summary(pls.fit)
dev.new()
...
set.seed (1)
pls.fit=plsr(fit.linear,data = data, subset=train, 
             scale=TRUE, validation ="CV")

validationplot(pls.fit,val.type="MSEP"); 
which.min(MSEP(pls.fit)$val[1,,][-1]); # M = 7
...
pls.fit=plsr(fit.linear,data = data ,scale=TRUE,ncomp=8)
summary(pls.fit)
