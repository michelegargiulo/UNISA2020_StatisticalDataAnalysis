...
pcr.fit=pcr(fit.poly3,data = data ,scale=TRUE, validation ="CV")
...
set.seed (1)
x = model.matrix(fit.poly3,data = data)[,-1]
...
pcr.fit=pcr(fit.poly3,data = data ,subset=train,scale=TRUE,
            validation ="CV")
dev.new()
# Plot MSE and RMSE 
validationplot(pcr.fit,val.type="MSEP",legendpos = "topright") 
...
# Setting scale=TRUE has the effect of standardizing each predictor (scale projection).
# Setting validation = 'CV' has the effect to use cross validation to rate M parameter
pls.fit=plsr(fit.poly3,data = data, scale=TRUE, 
             validation ="CV")
summary(pls.fit)
dev.new()
...
set.seed (1)
pls.fit=plsr(fit.poly3,data = data, subset=train, 
...
pls.fit=plsr(fit.poly3,data = data ,scale=TRUE,ncomp=22)
summary(pls.fit)
